...
class Critic(nn.Module):
    def __init__(self,s_size,a_size):
        ...
        self.fc1=nn.Linear(s_size+a_size,abs(s_size+a_size)//2)
        self.fc2=nn.Linear(abs(s_size+a_size)//2,1)
        """initial the weight"""
        self.fc1.weight.data.normal_(0,0.1)
        self.fc2.weight.data.normal_(0,0.1)
...
class DDPGAgent(ISoccerGameAgent):
    def __init__(self,env,gamma,lr_a,lr_c,batch_size,memory_capacity,tau):
        self.env=env
        self.s_size,self.a_size=len(env.state_space),env.action_space
        self.gamma,self.tau=gamma,tau
        self.batch_size=batch_size
        self.memory=np.zeros([memory_capacity, 2*self.s_size+1*self.a_size],dtype=np.float32) # This memory is expierience buffer, not the history of game
    ...
    def act(self,s_agent,s_opponent,s_ball):
        s=torch.tensor([s_agent,s_opponent,s_ball])
        a=self.actor(s).argmax(axis=0).detach()
        return a    # return a tensor with shape=(1,batch_a)
    
    
    def learn(self,alpha,s_agent,s_opponent,s_ball,a_agent,a_opponent,s_agent_,s_opponent_,s_ball_,r_agent,r_opponent,done):
        s=torch.tensor([s_agent,s_opponent,s_ball])
        s_=torch.tensor([s_agent_,s_opponent_,s_ball_])
    ...
    def update(self):
        ...
        q_target=batch_r+self.gamma * q_temp #! remeber the terminal
