        ...
        for episode in range(self.numEpisode):
            ...
            while True:
                ...
                if isinstance(self.agent,DDPGAgent):
                    with torch.no_grad():
                        q_value.append(self.agent.critic(torch.tensor(([[1,2,1]])) , [[3]])) # sample a certain sate-action
                        # pi_certain.append(self.agent.actor(torch.tensor(([[1,2,1]]))))
                elif isinstance(self.agent,QLearningAgent):
                        q_value.append(self.agent.q[74][3])
            if epsilon > self.epsilon_min:
                    epsilon *= self.epsilon_decay
            win_rate.append(np.average(memory))
        return win_rate,q_value#,pi_certain
