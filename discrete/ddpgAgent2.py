...
class DDPGAgent(ISoccerGameAgent):
    def __init__(self,env,gamma,lr_a,lr_c,batch_size,memory_capacity,tau):
        ...
        self.critic=Critic(self.s_size,self.a_span)
        self.actor_target=Actor(self.s_size,self.a_span)
        self.critic_target=Critic(self.s_size,self.a_span)
    ...
    def argonehot(self,pa): # to approximate the argmax
            ...
            return ((pa+c)*n) # still a tensor with shape(batch_size,a_span)
    ...
    def update(self):
        ...
        q_evalue=self.critic(batch_s,self.ext(batch_a))
